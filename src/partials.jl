...
@inline Base.:*(x::ResolvableType, partials::Partials) = partials*x
...
if NANSAFE_MODE_ENABLED
    ...
    @inline function Base.:*(partials::Partials, x::ResolvableType)
...
@inline Base.:*(partials::Partials{0,V}, x::ResolvableType) where {V} = Partials{0,promote_type(V,typeof(x))}(tuple())
#@inline Base.:*(x::Real, partials::Partials{0,V}) where {V} = Partials{0,promote_type(V,typeof(x))}(tuple())
@inline Base.:*(x::ResolvableType, partials::Partials{0,V}) where {V} = Partials{0,promote_type(V,typeof(x))}(tuple())
